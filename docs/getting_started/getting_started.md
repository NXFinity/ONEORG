# Getting Started
## How to use the ONEORG application

[[BACK](../index.md)] [[NEXT]](installation.md)

The easiest way to use the [ONEORG](https://oneorg.uk) application is by visiting [ONEORG](https://oneorg.uk) and registering for an account. Once you have registered, you can log in and start using the application.
This includes the ability to deploy your own solutions, such as the 'OneOrg Media Engine', across various deployments including locally using WSL2. Head over to [Installation](installation.md), on how to install the application locally.

## Installed Frameworks, Packages and Libraries
```
   @angular-devkit/build-angular (executors)    
   @angular/animations ()                       
   @angular/cdk (generators)                    
   @angular/cli ()                              
   @angular/common ()                           
   @angular/compiler ()                         
   @angular/compiler-cli ()                     
   @angular/core (generators)                   
   @angular/forms ()                            
   @angular/language-service ()                 
   @angular/localize (generators)               
   @angular/material (generators)               
   @angular/platform-browser ()                 
   @angular/platform-browser-dynamic ()         
   @angular/router ()                           
   @compodoc/compodoc (generators)              
   @cypress/schematic (executors,generators)    
   @jnxplus/nx-maven (executors,generators)     
   @jscutlery/semver (executors,generators)     
   @monodon/rust (executors,generators)         
   @nestjs/schematics (generators)              
   @ng-bootstrap/ng-bootstrap (generators)
   @nx-tools/nx-container (executors,generators)
   @nx/angular (executors,generators)
   @nx/cypress (executors,generators)
   @nx/detox (executors,generators)
   @nx/esbuild (executors,generators)
   @nx/eslint-plugin ()
   @nx/expo (executors,generators)
   @nx/express (generators)
   @nx/jest (executors,generators)
   @nx/js (executors,generators)
   @nx/linter (executors,generators)
   @nx/nest (generators)
   @nx/next (executors,generators)
   @nx/node (generators)
   @nx/plugin (executors,generators)
   @nx/react (executors,generators)
   @nx/react-native (executors,generators)
   @nx/rollup (executors,generators)
   @nx/storybook (executors,generators)
   @nx/vite (executors,generators)
   @nx/web (executors,generators)
   @nx/webpack (executors,generators)
   @nx/workspace (executors,generators)
   @nxext/capacitor (executors,generators)
   @nxext/ionic-angular (generators)
   @nxlv/python (executors,generators)
   @schematics/angular (generators)
   @twittwer/compodoc (executors,generators)
   angular-calendar (generators)
   ng2-charts (generators)
   ngx-bootstrap (generators)
   ngx-deploy-docker (executors,generators)
   ngx-deploy-npm (executors,generators)
   nx (executors)
   nx-cloud (generators)
```
Full Package List: [package.json](../../package.json)
```markdown
  "dependencies": 
    "@angular/animations": "~16.2.0",
    "@angular/cdk": "^16.2.0",
    "@angular/common": "~16.2.0",
    "@angular/compiler": "~16.2.0",
    "@angular/core": "~16.2.0",
    "@angular/forms": "~16.2.0",
    "@angular/language-service": "^16.2.0",
    "@angular/localize": "^16.2.0",
    "@angular/material": "^16.2.8",
    "@angular/platform-browser": "~16.2.0",
    "@angular/platform-browser-dynamic": "~16.2.0",
    "@angular/router": "~16.2.0",
    "@awesome-cordova-plugins/core": "^6.4.0",
    "@awesome-cordova-plugins/keyboard": "^6.4.0",
    "@awesome-cordova-plugins/splash-screen": "^6.4.0",
    "@awesome-cordova-plugins/status-bar": "^6.4.0",
    "@capacitor/core": "^5.5.0",
    "@casl/ability": "^6.5.0",
    "@faker-js/faker": "^8.1.0",
    "@fortawesome/fontawesome-free": "^6.4.2",
    "@fullcalendar/angular": "^6.1.9",
    "@fullcalendar/bootstrap": "^6.1.9",
    "@fullcalendar/core": "^6.1.9",
    "@fullcalendar/daygrid": "^6.1.9",
    "@fullcalendar/interaction": "^6.1.9",
    "@fullcalendar/list": "^6.1.9",
    "@fullcalendar/resource-timegrid": "^6.1.9",
    "@fullcalendar/timegrid": "^6.1.9",
    "@grpc/grpc-js": "^1.9.5",
    "@grpc/proto-loader": "^0.7.10",
    "@material-design-icons/svg": "^0.14.13",
    "@mui/icons-material": "^5.14.13",
    "@nestjs-modules/mailer": "^1.9.1",
    "@nestjs/axios": "^3.0.0",
    "@nestjs/bull": "^10.0.1",
    "@nestjs/cache-manager": "^2.1.0",
    "@nestjs/common": "^10.0.2",
    "@nestjs/config": "^3.1.1",
    "@nestjs/core": "^10.0.2",
    "@nestjs/cqrs": "^10.2.6",
    "@nestjs/event-emitter": "^2.0.2",
    "@nestjs/graphql": "^12.0.9",
    "@nestjs/jwt": "^10.1.1",
    "@nestjs/mapped-types": "^2.0.2",
    "@nestjs/microservices": "^10.2.7",
    "@nestjs/mongoose": "^10.0.1",
    "@nestjs/passport": "^10.0.2",
    "@nestjs/platform-express": "^10.2.7",
    "@nestjs/platform-socket.io": "^10.2.7",
    "@nestjs/schedule": "^3.0.4",
    "@nestjs/serve-static": "^4.0.0",
    "@nestjs/swagger": "^7.1.13",
    "@nestjs/terminus": "^10.1.1",
    "@nestjs/throttler": "^5.0.0",
    "@nestjs/typeorm": "^10.0.0",
    "@nestjs/websockets": "^10.2.7",
    "@ng-bootstrap/ng-bootstrap": "^15.1.1",
    "@ng-select/ng-select": "^11.1.1",
    "@ngneat/until-destroy": "^10.0.0",
    "@ngx-loading-bar/core": "^6.0.2",
    "@ngx-loading-bar/router": "^6.0.2",
    "@ngx-rocket/scripts": "^5.2.3",
    "@ngx-translate/core": "^15.0.0",
    "@nx/angular": "^16.10.0",
    "@nx/cypress": "^16.9.1",
    "@nx/detox": "^16.10.0",
    "@nx/esbuild": "^16.10.0",
    "@nx/expo": "^16.10.0",
    "@nx/express": "^16.10.0",
    "@nx/nest": "^16.10.0",
    "@nx/next": "^16.10.0",
    "@nx/plugin": "^16.10.0",
    "@nx/react": "^16.10.0",
    "@nx/react-native": "^16.10.0",
    "@nx/rollup": "^16.10.0",
    "@nx/storybook": "^16.10.0",
    "@nx/vite": "^16.10.0",
    "@nx/web": "^16.10.0",
    "@nx/webpack": "^16.10.0",
    "@popperjs/core": "^2.11.8",
    "@prisma/client": "^5.4.2",
    "@vegardit/prisma-generator-nestjs-dto": "^1.5.1",
    "amqp-connection-manager": "^4.1.14",
    "amqplib": "^0.10.3",
    "angular-calendar": "^0.31.0",
    "angulartics2": "^12.2.1",
    "apexcharts": "^3.43.2-0",
    "apollo-server-express": "^3.12.1",
    "async-sema": "^3.1.1",
    "axios": "^1.5.1",
    "badge-maker": "^3.3.1",
    "bcrypt": "^5.1.1",
    "blockstore-datastore-adapter": "^5.0.0",
    "bootstrap": "^5.3.2",
    "bootstrap-icons": "^1.11.1",
    "bootstrap-social": "^5.1.1",
    "bull": "^4.11.4",
    "cache-manager": "^5.2.4",
    "chalk": "^5.3.0",
    "chart.js": "^4.4.0",
    "class-transformer": "^0.5.1",
    "class-validator": "^0.14.0",
    "codelyzer": "^6.0.2",
    "compression": "^1.7.4",
    "cookie-parser": "^1.4.6",
    "cordova-android": "^12.0.0",
    "cordova-custom-config": "^5.1.1",
    "cordova-ios": "^7.0.0",
    "cordova-plugin-device": "^2.1.0",
    "cordova-plugin-ionic-keyboard": "^2.2.0",
    "cordova-plugin-ionic-webview": "^5.0.0",
    "cordova-plugin-splashscreen": "^6.0.2",
    "cordova-plugin-statusbar": "^3.0.0",
    "core-js": "^3.33.0",
    "csurf": "^1.11.0",
    "dart-sass": "^1.25.0",
    "datatables.net": "^1.13.4",
    "datatables.net-bs5": "^1.13.4",
    "date-fns": "^2.30.0",
    "discord.js": "^14.13.0",
    "dotenv": "^16.3.1",
    "electron-packager": "^17.1.2",
    "electron-reload": "^2.0.0-alpha.1",
    "express-session": "^1.17.3",
    "express-ws": "^5.0.2",
    "fibers": "^5.0.3",
    "fontawesome": "^5.6.3",
    "graphql": "^16.8.1",
    "graphql-tools": "^9.0.0",
    "hads": "^3.0.3",
    "helmet": "^7.0.0",
    "hls.js": "^1.4.12",
    "ioredis": "^5.3.2",
    "ipfs-repo": "^17.0.0",
    "it-buffer": "^0.1.3",
    "joi": "^17.11.0",
    "jquery": "^3.7.1",
    "js-yaml": "^4.1.0",
    "jsonwebtoken": "^9.0.2",
    "kafkajs": "^2.2.4",
    "libp2p": "^0.46.14",
    "libp2p-kad-dht": "^0.28.6",
    "lity": "^2.4.1",
    "lodash": "^4.17.21",
    "luxon": "^3.4.3",
    "masonry-layout": "^4.2.2",
    "material-design-icons-iconfont": "^6.7.0",
    "moment": "^2.29.4",
    "mongodb": "^6.1.0",
    "mongoose": "^7.6.2",
    "mpegts.js": "^1.7.3",
    "mqtt": "^5.1.2",
    "multiaddr": "^10.0.1",
    "multihash": "^0.1.0",
    "mysql": "^2.18.1",
    "nats": "^2.17.0",
    "nest-web3": "^1.1.3",
    "nestjs-ethers": "^2.1.0",
    "nestjs-prisma-graphql-crud-gen": "^1.0.4",
    "ng-apexcharts": "^1.8.0",
    "ng2-charts": "^5.0.3",
    "ngx-bootstrap": "^11.0.2",
    "ngx-chips": "^3.0.0",
    "ngx-color": "^9.0.0",
    "ngx-countdown": "^16.0.0",
    "ngx-deploy-docker": "^0.1.0",
    "ngx-editor": "^16.0.1",
    "ngx-highlightjs": "^10.0.0",
    "ngx-markdown": "^16.0.0",
    "ngx-mask": "^16.3.9",
    "ngx-masonry": "^14.0.1",
    "ngx-quicklink": "^0.4.2",
    "ngx-quill": "^23.0.0",
    "ngx-scrollbar": "^13.0.3",
    "node-rtsp-stream": "^0.0.9",
    "nodemailer": "^6.9.6",
    "npm-run-all": "^4.1.5",
    "passport": "^0.6.0",
    "passport-jwt": "^4.0.1",
    "passport-local": "^1.0.0",
    "peer-id": "^0.16.0",
    "peer-info": "^0.17.5",
    "pg": "^8.11.3",
    "popper": "^1.0.1",
    "popper.js": "^1.16.1",
    "prisma-class-generator": "^0.2.7",
    "prisma-class-validator-generator": "^0.2.0",
    "prisma-dbml-generator": "^0.10.0",
    "prisma-docs-generator": "^0.8.0",
    "prisma-joi-generator": "^0.2.0",
    "prisma-json-schema-generator": "^4.0.0",
    "prisma-json-server-generator": "^0.2.0",
    "prisma-redis-middleware": "^4.8.0",
    "protractor": "~7.0.0",
    "quill": "^1.3.7",
    "reflect-metadata": "^0.1.13",
    "rimraf": "^3.0.2",
    "rxjs": "~7.8.0",
    "rxjs-compat": "^6.6.7",
    "showdown": "^2.1.0",
    "simplebar": "^6.2.5",
    "socket.io": "^4.7.2",
    "sqlite3": "^5.1.6",
    "stripe": "^13.10.0",
    "swagger-ui-express": "^5.0.0",
    "swiper": "^10.3.1",
    "tslib": "^2.3.0",
    "typeorm": "^0.3.17",
    "uuid": "^9.0.1",
    "ws": "^8.14.2",
    "zone.js": "~0.13.0"

  "devDependencies": 
    "@angular-devkit/build-angular": "~16.2.0",
    "@angular-devkit/core": "~16.2.0",
    "@angular-devkit/schematics": "~16.2.0",
    "@angular-eslint/eslint-plugin": "~16.0.0",
    "@angular-eslint/eslint-plugin-template": "~16.0.0",
    "@angular-eslint/template-parser": "~16.0.0",
    "@angular/cli": "~16.2.0",
    "@angular/compiler-cli": "~16.2.0",
    "@angular/language-service": "~16.2.0",
    "@capacitor/android": "^5.0.3",
    "@capacitor/cli": "^5.0.3",
    "@capacitor/ios": "^5.0.3",
    "@compodoc/compodoc": "^1.1.19",
    "@cypress/schematic": "^2.5.0",
    "@faker-js/faker": "^7.6.0",
    "@jnxplus/nx-maven": "^0.10.1",
    "@jscutlery/semver": "^3.1.0",
    "@monodon/rust": "^1.2.0",
    "@nestjs/cli": "^10.0.0",
    "@nestjs/schematics": "^10.0.1",
    "@nestjs/testing": "^10.0.2",
    "@ngneat/until-destroy": "^10.0.0",
    "@ngx-rocket/scripts": "^5.2.3",
    "@nx-tools/container-metadata": "^5.0.3",
    "@nx-tools/nx-container": "^5.0.3",
    "@nx/cypress": "16.10.0",
    "@nx/eslint-plugin": "16.10.0",
    "@nx/jest": "16.10.0",
    "@nx/js": "16.10.0",
    "@nx/linter": "16.10.0",
    "@nx/node": "16.10.0",
    "@nx/workspace": "16.10.0",
    "@nxext/capacitor": "^16.7.0",
    "@nxext/ionic-angular": "^16.2.0",
    "@nxlv/python": "^16.1.2",
    "@schematics/angular": "~16.2.0",
    "@twittwer/compodoc": "^1.10.0",
    "@types/bcrypt": "^5.0.0",
    "@types/cookie-parser": "^1.4.3",
    "@types/datatables.net": "^1.10.24",
    "@types/express": "^4.17.17",
    "@types/express-session": "^1.17.7",
    "@types/jasmine": "~4.3.4",
    "@types/jasminewd2": "~2.0.10",
    "@types/jest": "^29.4.0",
    "@types/jquery": "^3.5.16",
    "@types/js-yaml": "^4.0.5",
    "@types/lodash": "^4.14.197",
    "@types/luxon": "^3.2.0",
    "@types/multer": "^1.4.7",
    "@types/node": "~18.7.1",
    "@types/passport-jwt": "^3.0.9",
    "@types/quill": "^1.3.10",
    "@types/showdown": "~1.9.4",
    "@types/supertest": "^2.0.12",
    "@typescript-eslint/eslint-plugin": "^5.60.1",
    "@typescript-eslint/parser": "^5.60.1",
    "autoprefixer": "^10.4.0",
    "cordova": "^12.0.0",
    "cypress": "^13.0.0",
    "daisyui": "^3.5.1",
    "electron": "^27.0.0",
    "electron-packager": "^17.1.2",
    "electron-reload": "^2.0.0-alpha.1",
    "eslint": "~8.46.0",
    "eslint-config-prettier": "8.1.0",
    "eslint-plugin-cypress": "^2.13.4",
    "eslint-plugin-import": "^2.28.1",
    "eslint-plugin-jsdoc": "^46.5.0",
    "eslint-plugin-prefer-arrow": "^1.2.3",
    "eslint-plugin-prettier": "^4.2.1",
    "hads": "^3.0.3",
    "https-proxy-agent": "^7.0.1",
    "husky": "^8.0.3",
    "jasmine-core": "~4.6.0",
    "jasmine-spec-reporter": "~5.0.0",
    "jest": "^29.4.1",
    "jest-environment-jsdom": "^29.4.1",
    "jest-environment-node": "^29.4.1",
    "jest-preset-angular": "~13.1.0",
    "jsonc-eslint-parser": "^2.1.0",
    "jsvectormap": "^1.5.3",
    "karma": "~6.4.0",
    "karma-chrome-launcher": "~3.2.0",
    "karma-coverage": "~2.2.0",
    "karma-jasmine": "~5.1.0",
    "karma-jasmine-html-reporter": "~2.0.0",
    "ngx-deploy-docker": "^0.1.0",
    "ngx-deploy-npm": "^7.0.1",
    "nx": "16.10.0",
    "nx-cloud": "latest",
    "postcss": "^8.4.5",
    "postcss-scss": "~4.0.4",
    "prettier": "^2.6.2",
    "pretty-quick": "^3.1.3",
    "prisma": "^5.1.0",
    "puppeteer": "^21.1.0",
    "run-script-webpack-plugin": "^0.2.0",
    "source-map-support": "^0.5.21",
    "stylelint": "^15.10.3",
    "stylelint-config-prettier": "^9.0.5",
    "stylelint-config-recommended-scss": "^12.0.0",
    "stylelint-config-standard": "^34.0.0",
    "supertest": "^6.3.3",
    "tailwindcss": "^3.0.2",
    "ts-jest": "^29.1.0",
    "ts-loader": "^9.4.3",
    "ts-node": "10.9.1",
    "tsconfig-paths": "^4.2.0",
    "typescript": "~5.1.3",
    "video.js": "^8.3.0",
    "webpack": "^5.88.2",
    "webpack-node-externals": "^3.0.0"
```
